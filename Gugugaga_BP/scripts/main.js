...
world.afterEvents.entityHurt.subscribe((event) => {
    ...
    // Penguin đánh trúng kẻ thù → kích hoạt animation đánh 1 lần
    if (attacker?.typeId === "custom:gugugaga_penguin") {
        try {
            attacker.setProperty("gugugaga:is_attacking", true);
            system.runTimeout(() => {
                try { attacker.setProperty("gugugaga:is_attacking", false); } catch(e) {}
            }, 10);
        } catch(e) {}
    }
});
...
world.afterEvents.playerInteractWithEntity.subscribe((event) => {
    ...
    if (health) {
        ...
        // Kích hoạt animation beg (vui) sau khi ăn cookie
        try {
            target.setProperty("gugugaga:is_happy", true);
            system.runTimeout(() => {
                try { target.setProperty("gugugaga:is_happy", false); } catch(e) {}
            }, 60);
        } catch(e) {}
    }
    ...
    return;
        ...
        // Kích hoạt animation beg (vui) khi chủ tương tác mở UI
        try {
            target.setProperty("gugugaga:is_happy", true);
            system.runTimeout(() => {
                try { target.setProperty("gugugaga:is_happy", false); } catch(e) {}
